...
let leaveNominators = async (api, amount) => {
  ...
  if (
    stakingBalance &&
    new BN(stakingBalance.stakedBalance).gt(new BN(stakingBalance.unlockedBalance).add(new BN(stakingBalance.unstakedBalance)))
  ) {
  ...
  if (new BN(stakingBalance && stakingBalance.unstakedBalance).gt(new BN(0))) await unlockStakedBalance(api);
