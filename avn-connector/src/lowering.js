...
async function getLowerTransactions(blockNumber) {
  ...
  const txQueryResponse = await axios.post(AVN_EXPLORER_URL, { txQuery, operationName: 'ConnectorLowerTx' });
  ...
  const statusQueryResponse = await axios.post(AVN_EXPLORER_URL, { query, operationName: 'ConnectorLowerStatus' });
