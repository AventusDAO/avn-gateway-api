...
async function updateNodeModulesAndPublish(lambda, zip) {
  try {
    ...
    await publish(lambda, layers, zip)
...
async function publish(lambda, layers, zip) {
  ...
  await publishLambdaLayer(lambda, description, layers, zip)
  await publishSourceCode(lambda, zip)
...
async function cleanUpOldLayerVersions(layerName, version, zip) {
  while (!zip && --version > 0) {
    await aws.send(
