import { axios, init, callWithTimeout, requestFailed, buildErrorResponse, buildErrorBody,
...
export const handler = async (event: APIGatewayProxyEvent, context: Context): Promise<APIGatewayProxyResult> => {
  await init();
  const result = await callWithTimeout(context.getRemainingTimeInMillis(), processRequest, [event, context]);

  if (requestFailed(result)) {
    return buildErrorResponse(500, result.error.data, JSON.stringify(result));
  }

  return buildSuccessResponse(JSON.stringify(result));
};
...
async function processRequest(event: APIGatewayProxyEvent, context: Context): Promise<any> {
  ...
  try {
    ...
    if (isSplitFeeTransaction(authoriserContext) === true) {
      const eventType = WEBHOOK_EVENT_TYPES.tx_received;
      await publishEvent(AVN_CONNECTOR_ENDPOINT, eventType, awsRequestId, authoriserContext.splitFeePayerAddress, tx);
    ...
    return buildValidResponseBody(tx.id, awsRequestId);
  } catch (err: any) {
    // Let the caller know that this transaction has failed to be sent to the chain
    await axios.post(`${AVN_CONNECTOR_ENDPOINT}setTransactionFailedToBeSentStatus`, { requestId: awsRequestId });

    return buildErrorBody('internal', 'Failed to handle send transaction', err.toString(), request, tx.id);
  }
}
...
async function sendMessageToPayerQueue(tx: Transaction, request: string, awsRequestId: string, authoriserContext: ValidRequestContext): Promise<any> {
  if (tx.params && Array.isArray(tx.params) ? !!tx.params[0].feePaymentSignature : !!tx.params.feePaymentSignature) {
...
function isSplitFeeTransaction(authoriserContext: ValidRequestContext): boolean {
  ...
  return authoriserContext.isSplitFeeUser === true && isValidAccountId(authoriserContext.splitFeePayerAddress);
}
