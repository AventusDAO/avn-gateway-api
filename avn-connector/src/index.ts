...
import redis, {TxStatus} from './redis';
...
app.post('/setTransactionRefusedByPayerStatus', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await avn.setSendingFailedStatus(req.body.requestId, TxStatus.PayerRefused);
    res.status(200).send({});
  } catch (err) {
    next(err);
  }
...
app.post('/setTransactionFailedToBeSentStatus', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await avn.setSendingFailedStatus(req.body.requestId, TxStatus.SendingFailed);
    res.status(200).send({});
  } catch (err) {
    next(err);
  }
