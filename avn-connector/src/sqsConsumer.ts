  ...
  switch (txType) {
    case 'avnProxy':
      ...
      if (isSplitFeeTransaction(txData)) {
        ...
        params.paymentNonce = await avn.getPayerPaymentNonce(
          requestId,
          payerAddress
        );
        logger.info(
          `[SQS tx] Request ID: ${requestId} - split fee payment nonce: ${params.paymentNonce}`
        );
    ...
    default:
      logger.error(
        `[SQS tx] Request ID: ${requestId} - Unsupported transaction type: "${txType}"`
      );
  }
}

async function deleteMessages(messages: Message[]): Promise<number> {
  let entries = messages.map(message => ({
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    if (attempt === 1) {
      const deletedMessages = new Set(
        result.Successful!.map(message => message.Id)
      );
      entries = entries.filter(entry => !deletedMessages.has(entry.Id));
    } else {
      logger.error(
        '[SQS tx] Failed to delete processed messages after retry:',
        result.Failed
      );
    }
  }
